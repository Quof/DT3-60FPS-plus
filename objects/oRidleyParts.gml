...
else if type=1 //----- Wings -----
{
  ...
  else if myAnim>=24 and myAnim<=30 {image_index=0; myAnim=0}
...
else if type=3 //----- Tail -----
{
  ...
  else if tailType=2 //Point down
  {
    ...
    {
      if myOwner.image_xscale=1 {image_angle=270}
      else {image_angle=90}
    }
  }
  ...
  else if tailType=4 //Extend toward player
  {
    x=myOwner.x-(43*image_xscale)+lengthdir_x(tailSeg*tailSep,tailAngle)
    y=myOwner.y+29+lengthdir_y(tailSeg*tailSep,tailAngle)
    if sprite_index=sRidleyTailEnd
    {
      if myOwner.image_xscale=1 {image_angle=tailAngle}
      else {image_angle=tailAngle+180}
