...
if global.gamePaused=false
{
  ...
  if life<=0 //Defeat animation
  {
    ...
    {
      if x<(room_width/2) {x+=16*gDeltaTime}
      else {x-=16*gDeltaTime}
      if y<224 {y+=16*gDeltaTime}
      else {y-=16*gDeltaTime}
...
if attackTime>=1 and attackTime<=60 //Track player if player is too far
{
  if playerDist>=40
  {
    if x<oPlayer1.x {x+=2*gDeltaTime}
    else {x-=2*gDeltaTime}
  }
}
