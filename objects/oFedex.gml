...
eName="Fedex" //CHAOS Warmaster EX EX
eLevel=40
maxLife=16500
life=maxLife
atkPower=12
...
fireFreq=4
fireDuration=190
FASTER_CHANGE=3 //Fedex change from 2 -> 3
weaknessChange=0
pepsiLogo=0
//--------------------
//DELETE THIS
...
if global.gamePaused=false
{
  if activateBoss=1
  {
    ...
    jeremyText="You fought WEX. This variant has him with no down time between any attacks and his form change order is now completely RNG. His player state based decision making is now RNG as well. He can have more than one Wind Orb. He is in WEX's sped up post-50% mode for the entire fight and he has 100 more HP."
    chaoText="On top of what Jeremy said, your Dash Energy also restores just a hair bit faster on this than WEX. Even though this isn't in the boss gallery, you can still use the quick boss retry. This fight is not timed."
    devText="Guys, stop playing this game.##Also there is no scan text variations on forms here. Complain to me about it on Discord. <3"
    activateBoss=2
  }
...
if currentAttack=1 //==================== ATK A: SWORD SWING 1 - (Leads into Atk B) ==================== EX
{
  ...
  else //EX MODE
  {
    ...
    if actTime>=20 and actTime<=26 and EX_SWORDSWING=1 and gDeltaDoTicks {instance_create(x,y,oWMA_EX_DashImage)} //Dash back effect
  }
}
...
else if currentAttack=6 //==================== ATK F: ICE SHOT TO PLAYER ==================== EX
{
  ...
  else if actTime=14
  {
    ...
    else //EX MODE
    {
      ...
      tAtk.atkPower=atkPower; tAtk.bulletSpeed=7 tAtk.posX=tIcePosX; tAtk.posY=tIcePosY; tAtk.spreadDir=tIceSpreadStart; tAtk._direction=tIceDir
    }
  }
  else if actTime>=20
  {
...
if ATTACK_FORM=1 //======================================== MAIN FORM ========================================
{
  if atkNum=0 //----- First attack -----
  {
    ...
    {
      if choose(1,2)=1 //Player is not attacking
      {
...
else if ATTACK_FORM=4 //======================================== WIND FORM ========================================
{
  ...
  else if atkNum>=2 //----- Third attack -----
  {
    ...
    else //-- Player is far --
    {
      if atkInSequence=0 or atkInSequence=1
      {
        if choose(1,2)=1 {tornadoNum=0; currentAttack=4}
        else
        {
          tornadoNum+=1
          if tornadoNum<=1 {currentAttack=3}
