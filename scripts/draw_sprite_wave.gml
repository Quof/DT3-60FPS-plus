...
for (i=0; i<size; i+=1) {
  ...
  if (axis) {
    sx = shift-xoff+posx;
    sy = i-yoff+posy;
    draw_sprite_part_ext(sprite,image,0,i,width,1,sx,sy,image_xscale,image_yscale,image_blend,image_alpha);
  }else{
    sx = i-xoff+posx;
    sy = shift-yoff+posy;
    draw_sprite_part_ext(sprite,image,i,0,1,height,sx,sy,image_xscale,image_yscale,image_blend,image_alpha);
  }
}
